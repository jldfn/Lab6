...
public class ConsoleApp {
    ...
    public static Human ParseJSON(String parseString, JTextPane field) {
        String[] sepString = parseString.split(" ");
        //Scanner itemReader = new Scanner(System.in);
        String str="Начат парсинг";
        try{
        if(sepString[0].contains("\"Human\":{")){
                    ...
                    switch(buffString[0]){
                        case "\"name\"": Matcher hName=Pattern.compile("[0-9]").matcher(buffString[1]);
                            if(hName.find()){System.out.println("Поле name может содержать исключительно символы латинского алфавита и кириллицы");
                                            str="Поле name может содержать исключительно символы латинского алфавита и кириллицы";
                                            }
                            else{if(name!=null) {
                                System.out.println("Атрибут name введён повторно, будет использовано следующее значение: " + buffString[1]);
                                str="Атрибут name введён повторно, будет использовано следующее значение: " + buffString[1];
                            }
                        ...
                        case "\"age\"":Matcher hAge=Pattern.compile("\"[0-9]+\"").matcher(buffString[1]);
                            if(hAge.matches()){
                                ...
                                str="Атрибут age введён несколько раз, будет использовано последнее значение: "+buffString[1];
                            }else
                            {System.out.println("В возрасте могут присутствовать только цифры!");
                            str="В возрасте могут присутствовать только цифры!";
                            }break;
                        case "\"loc\"":if(location!=null){
                            System.out.println("Атрибут location введён несколько раз, будет использовано корректное последнее значение"+buffString[1]);
                            str="Атрибут location введён несколько раз, будет использовано корректное последнее значение"+buffString[1];
                        }
                        ...
                        str="В классе Human нет поля "+buffString[0];
                        }
                    }
                }
                ...
                str = "Атрибут name не был задан";
                    ...
                    str = "Атрибут location не был задан";
                    }
                }
                }else{System.out.println("Объекты данного типа не могут быть занесены в коллекцию");
        str = "Объекты данного типа не могут быть занесены в коллекцию";
        }
        }catch(IndexOutOfBoundsException e){System.out.println("Ошибка преобразования объекта");
        str = "Ошибка преобразования объекта";
        }
        catch(NumberFormatException l){System.out.println("Ошибка преобразования численного поля");
        str = "Ошибка преобразования численного поля";
        }
        field.setText(field.getText()+System.getProperty("line.separator")+str);
        return (null);
    }
